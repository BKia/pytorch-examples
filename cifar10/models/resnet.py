...
class BasicNoBNBlock(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(BasicNoBNBlock, self).__init__()
...
class CifarNoBNNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, reinit_std=None):
        super(CifarNoBNNet, self).__init__()
        self.in_planes = 16
...
def CifarResNetNoBNBasic(num_blocks, reinit_std=None):
    assert len(num_blocks) == 3, "3 blocks are needed, but %d is found." % len(num_blocks)
    return CifarNoBNNet(BasicNoBNBlock, num_blocks, reinit_std=reinit_std)
